...
def test_case_2():
    ...
    assert f'{type(circular_queue_0).__module__}.{type(circular_queue_0).__qualname__}' == 'circular_queue.CircularQueue'
    assert len(circular_queue_0) == 0
    var_0 = circular_queue_0.enqueue(none_type_0)
    assert len(circular_queue_0) == 1
    assert f'{type(var_0).__module__}.{type(var_0).__qualname__}' == 'circular_queue.CircularQueue'
    assert len(var_0) == 1
