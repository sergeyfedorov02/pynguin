...
def test_case_0():
    ...
    assert (
        f"{type(disjoint_set_0).__module__}.{type(disjoint_set_0).__qualname__}"
        == "alternate_disjoint_set.DisjointSet"
    )
    assert disjoint_set_0.set_counts == [True, True, True]
    assert disjoint_set_0.max_set is True
    assert disjoint_set_0.ranks == [1, 1, 1]
    assert disjoint_set_0.parents == [0, 1, 2]
    bool_1 = disjoint_set_0.merge(bool_0, bool_0)
    assert bool_1 is False
    bool_2 = disjoint_set_0.merge(bool_0, bool_1)
    assert disjoint_set_0.set_counts == [2, 0, True]
    assert disjoint_set_0.max_set == 2
    assert disjoint_set_0.ranks == [2, 1, 1]
    assert disjoint_set_0.parents == [0, False, 2]


def test_case_1():
    bool_0 = False
    tuple_0 = (bool_0,)
    disjoint_set_0 = module_0.DisjointSet(tuple_0)
    assert (
        f"{type(disjoint_set_0).__module__}.{type(disjoint_set_0).__qualname__}"
        == "alternate_disjoint_set.DisjointSet"
    )
    assert disjoint_set_0.set_counts == (False,)
    assert disjoint_set_0.max_set is False
    assert disjoint_set_0.ranks == [1]
    assert disjoint_set_0.parents == [0]
...
def test_case_2():
    ...
    assert disjoint_set_0.ranks == [2, 1, 1]
    assert disjoint_set_0.parents == [0, False, 2]
    bool_3 = disjoint_set_0.merge(bool_2, bool_1)
