# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pytest
import min_heap as module_0


def test_case_0():
    str_0 = ""
    min_heap_0 = module_0.MinHeap(str_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == ""


@pytest.mark.xfail(strict=True)
def test_case_1():
    bytes_0 = b"6M]\x93\xe3\xb7"
    module_0.MinHeap(bytes_0)


@pytest.mark.xfail(strict=True)
def test_case_2():
    none_type_0 = None
    module_0.MinHeap(none_type_0)


def test_case_3():
    none_type_0 = None
    node_0 = module_0.Node(none_type_0, none_type_0)


@pytest.mark.xfail(strict=True)
def test_case_4():
    bool_0 = False
    node_0 = module_0.Node(bool_0, bool_0)
    var_0 = node_0.__str__()
    assert var_0 == "Node(False, False)"
    var_0.get_parent_idx(node_0)


@pytest.mark.xfail(strict=True)
def test_case_5():
    none_type_0 = None
    node_0 = module_0.Node(none_type_0, none_type_0)
    node_0.__lt__(node_0)


@pytest.mark.xfail(strict=True)
def test_case_6():
    tuple_0 = ()
    min_heap_0 = module_0.MinHeap(tuple_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == ()
    none_type_0 = None
    min_heap_0.insert(none_type_0)


@pytest.mark.xfail(strict=True)
def test_case_7():
    int_0 = 850
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = min_heap_0.is_empty()
    assert var_0 is True
    min_heap_0.sift_down(int_0, int_0)


@pytest.mark.xfail(strict=True)
def test_case_8():
    str_0 = ""
    min_heap_0 = module_0.MinHeap(str_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == ""
    bool_0 = True
    min_heap_0.__getitem__(bool_0)


@pytest.mark.xfail(strict=True)
def test_case_9():
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    min_heap_0.peek()


@pytest.mark.xfail(strict=True)
def test_case_10():
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = min_heap_0.is_empty()
    assert var_0 is True
    var_1 = set_0.__str__()
    var_2 = min_heap_0.get_parent_idx(var_0)
    assert var_2 == 0
    var_2.get_left_child_idx(var_2)


@pytest.mark.xfail(strict=True)
def test_case_11():
    bool_0 = False
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = min_heap_0.sift_up(bool_0)
    min_heap_0.remove()


@pytest.mark.xfail(strict=True)
def test_case_12():
    bool_0 = True
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    min_heap_0.sift_up(bool_0)


def test_case_13():
    bool_0 = False
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = min_heap_0.sift_up(bool_0)


@pytest.mark.xfail(strict=True)
def test_case_14():
    bool_0 = False
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = set_0.__str__()
    dict_0 = {var_0: min_heap_0}
    var_1 = var_0.__lt__(min_heap_0)
    var_2 = min_heap_0.sift_down(bool_0, dict_0)
    node_0 = module_0.Node(bool_0, dict_0)
    bool_0.remove()


@pytest.mark.xfail(strict=True)
def test_case_15():
    bool_0 = False
    node_0 = module_0.Node(bool_0, bool_0)
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = node_0.__str__()
    assert var_0 == "Node(False, False)"
    var_1 = min_heap_0.sift_down(bool_0, var_0)
    var_1.remove()


@pytest.mark.xfail(strict=True)
def test_case_16():
    bool_0 = True
    node_0 = module_0.Node(bool_0, bool_0)
    set_0 = set()
    min_heap_0 = module_0.MinHeap(set_0)
    assert (
        f"{type(min_heap_0).__module__}.{type(min_heap_0).__qualname__}"
        == "min_heap.MinHeap"
    )
    assert min_heap_0.idx_of_element == {}
    assert min_heap_0.heap_dict == {}
    assert min_heap_0.heap == {*()}
    var_0 = node_0.__str__()
    assert var_0 == "Node(True, True)"
    min_heap_0.sift_down(bool_0, var_0)


@pytest.mark.xfail(strict=True)
def test_case_17():
    bool_0 = False
    str_0 = 'Gb"TD\t\rp23&A#Q'
    node_0 = module_0.Node(str_0, str_0)
    node_1 = module_0.Node(bool_0, bool_0)
    set_0 = {node_1, node_0}
    module_0.MinHeap(set_0)
